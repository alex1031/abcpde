...
def generate_solution(nx, ny, Lx, Ly, cx, cy, s):
    ...
    sol = sol[22:27, 22:27, :] # We only want to compare the relevant parts and match the components of the observed
    return np.array(sol)
...
def abc_simulation(observed, n=100): # Performs Approximate Bayesian Computation (ABC) simulation. Returns results and timing information
    ...
    for i in range(n):
        ...
        simulated = generate_solution(NX, NY, LX, LY, cx, cy, s[i])
