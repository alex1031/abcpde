...
from gaussian_plume.abc_simulation_diffusion import main
import time
...
def generate_paths(noise: str) -> List[Tuple[str, str, str]]:
    ...
    """
    observed_path = os.path.join(OBSERVED_DIR, f"{noise}_noise_diffusion", f"{noise}_noise_diffusion.npy")
    output_dir = os.path.join(SAVE_DIR, f"{noise}_noise_diffusion")
