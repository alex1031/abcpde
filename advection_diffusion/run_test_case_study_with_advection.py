...
from gaussian_plume.abc_simulation_case_study_with_advection import main
import time
...
def generate_paths(normalised=False) -> List[Tuple[str, str, str]]:
    ...
    if normalised:
        ...
        output_dir = os.path.join(SAVE_DIR, "case_study_with_advection_normalised")
    else:    
        observed_path = os.path.join(OBSERVED_DIR, "case_study", "case_study.npy")
        output_dir = os.path.join(SAVE_DIR, "case_study_with_advection")
...
def run_simulation(args: Tuple[str, str, str]) -> None:
    ...
    try:
        ...
        main(observed_path, output_path, time_output_path, normalised=True)
...
if __name__ == "__main__":
    ...
    task_args = generate_paths(normalised=True)
