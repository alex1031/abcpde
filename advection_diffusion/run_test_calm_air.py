...
from gaussian_plume.abc_simulation_calm_air import main
import time
...
def generate_paths(noise: str) -> List[Tuple[str, str, str]]:
    ...
    """
    observed_path = os.path.join(OBSERVED_DIR, f"{noise}_noise_calm_air", f"{noise}_noise_calm_air.npy")
    output_dir = os.path.join(SAVE_DIR, f"{noise}_noise_calm_air")
