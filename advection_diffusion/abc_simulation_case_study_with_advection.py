...
def compute_frechet_distance(sim, obs):
    return np.mean([frechet_distance(sim, obs)])
...
def abc_simulation(observed, n=100, normalised=False): # Performs Approximate Bayesian Computation (ABC) simulation. Returns results and timing information
    ...
    cx, cy = rng.uniform(0, 5, n), rng.uniform(0, 5, n) # Moderate wind conditions.
    s = rng.uniform(0, 1e-4, n)

    for i in range(n):
        start_time = time.time()
        simulated = generate_solution(NX, NY, LX, LY, cx[i], cy[i], s[i])[-1] # We now compare the final state of the plume, i.e. the last iteration of the solution.
        if normalised:
