...
from gaussian_plume.abc_simulation_5e3_advection import main
import time
...
def generate_paths(noise: str) -> List[Tuple[str, str, str]]:
    ...
    """
    observed_path = os.path.join(OBSERVED_DIR, f"{noise}_noise_5e-3_advection", f"{noise}_noise_5e-3_advection.npy")
    output_dir = os.path.join(SAVE_DIR, f"{noise}_noise_5e-3_advection")
