...
if output == PLAN_VIEW or output == SURFACE_TIME or output == OVERLAY:
...
if output == PLAN_VIEW:
   if not os.path.exists('/tmp/' + username):
      os.makedirs('/tmp/' + username, exist_ok=True)
   ...
   import overlay_on_map
   overlay_on_map.overlay_on_map(x,y,C1)
else:
   sys.exit()
