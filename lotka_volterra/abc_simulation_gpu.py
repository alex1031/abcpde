...
def simulation_uniform(observed_prey: np.ndarray, observed_predator: np.ndarray, niter: int=1000) -> np.ndarray:
    # Initial conditions
    theta_a, theta_b = torch.randint(-10, 10, size=(1,niter)).cuda(), torch.randint(-10, 10, size=(1,niter)).cuda()
    ...
    ic = torch.full((niter, 2), 0.5).cuda() # initial conditions
    t = torch.linspace(0, 10, 100).cuda()
    sim_sol = odeint(dUdt, ic, t, method='rk4')
    ...
    results = np.column_stack((theta_a.cpu().T, theta_b.cpu().T, wasserstein, energy, mmd, cramer, kld))
